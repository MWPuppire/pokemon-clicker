...
var addTown = function(name,gym,image,shop,reqRoutes,extras){
    var temp = {
        name: name,
        gym: gym,
        image: image,
...
var showTown = function(town){
    ...
    if(town.shop === "oak"){
        if(player.mapExplain || player.townExplain || player.dungeonExplain) {
            html += "<button onClick='oakExplainAgain()' class='tutorial leftTownButton btn btn-primary col-sm-2' id='"+town.name+" oak'>Oak</button>";
    ...
    if(town.gym != null){
        if(town.gym.bossList != undefined){
            ...
            if(player.gymBadges.length >= town.gym.badgeReq){
                if (town.gym.tokenCost > 0) {
                    html += "<button class='dungeon leftTownButton btn btn-primary col-sm-2' id='"+town.name+" Dungeon'>Dungeon</button>"
                } else {
                    html += "<button class='dungeon leftTownButton btn btn-primary col-sm-2' id='"+town.name+" Dungeon'>Dungeon<br>"+town.gym.moneyCost+" coins</button>"
                };
            } else {
    ...
    if (town.extras != "" && getTown(town.extras)) {
        if (isActive(town.extras + " Pass")) {
            html += "</div><div class='row'>";
            html += "<button class='leftTownButton btn btn-primary col-sm-2' id='"+town.extras+"'>"+town.extras+"</button>";
        }
    }
...
var loadTowns = function(){
    ...
    addTown("Vermillion City",VermillionCityGym(), "images/gyms/vermillioncity.png", VermillionCityShop(), [6], "S.S. Anne");
    addTown("Celadon City", CeladonCityGym(), "images/gyms/celadoncity.png", CeladonCityShop(), [8]);
    addTown("Saffron City", SaffronCityGym(), "images/gyms/saffroncity.png", SaffronCityShop(), [5], "Magnet Train");
    ...
    addTown("Pallet Town", null, "images/gyms/pallettown.png", "oak", []);
    ...
    addTown("Champion Blue", Champion(), null, null, null);
    ...
    addTown("Safari Zone", SafariZoneDungeon(), "images/dungeons/safarizone.png", null, [18,15]);

    addTown("Magnet Train", null, "images/places/magnettrain.png", null, [5]);
    addTown("S.S. Anne", null, "images/places/ssanne.png", null, [6]);
}
