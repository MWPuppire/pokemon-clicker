...
var Dungeon = function(name, pokemons, size, baseHealth, bossList, tokenCost, badgeReq, itemRoute, level, moneyCost) {
    var temp = {
        name: name,
        pokemonDefeated: 0,
        size: size,
...
var PowerPlantDungeon = function(){
    ...
    var bossPokemon = [BossPokemon("Zapdos", 101302, 50), BossPokemon("Electabuzz", 67535, 35), BossPokemon("Electabuzz", 67535, 35)];
...
var SeafoamIslandsDungeon = function(){
    ...
    var bossPokemon = [ BossPokemon("Articuno", 129195, 50) , BossPokemon("Seel", 86130, 35), BossPokemon("Seel", 86130, 35)];
...
var VictoryRoadDungeon = function(){
    ...
    var bossPokemon = [BossPokemon("Moltres", 184462, 50), BossPokemon("Machoke", 122975, 42), BossPokemon("Machoke", 122975, 42)];
...
var CeruleanCaveDungeon = function(){
    ...
    var bossPokemon = [BossPokemon("Mewtwo", 215512, 70), BossPokemon("Rhydon", 143675, 60), BossPokemon("Rhydon", 143675, 60)];
    return Dungeon("Cerulean Cave Dungeon", pokemonList, 5, 28735, bossPokemon, 2500, 8, 20, 55);
}

var SafariZoneDungeon = function(){
    var pokemonList = ["Nidorina", "Nidorina", "Nidorino", "Nidorino", "Exeggcute", "Rhyhorn", "Doduo", "Doduo", "Parasect", "Parasect", "Venomoth", "Venomoth", "Chansey", "Scyther", "Kangaskhan", "Pinsir", "Tauros"];
    var bossPokemon = [BossPokemon("Rhydon", 132575, 60)];
    return Dungeon("Safari Zone Dungeon", pokemonList, 7, 25775, bossPokemon, 0, 8, 20, 40, 50000);
}

var loadDungeon = function(townId) {
        currentDungeon = getTown(townId).gym;
        $("#dungeonModal").modal('hide');
    if(player.money >= currentDungeon.moneyCost){
        ...
        player.money -= currentDungeon.moneyCost;
        currentDungeon.loot = [];
        dungeonCanMove = 1;
        updateDungeon();
        ...
        $.notify("You don't have enough money to start this dungeon","error");
    }
}
...
var showDungeonDefeated = function() {
    if(inProgress === 3){
        ...
        html += "<div class='row'>"
        if (currentDungeon.tokenCost > 0) {
            html +=     "<button class='dungeon leftTownButton btn btn-primary col-sm-2' id='" + currentDungeon.name + "'>Retry!</button>"
        } else {
            html +=     "<button class='dungeon leftTownButton btn btn-primary col-sm-2' id='" + currentDungeon.name + "'>Retry!<br>"+currentDungeon.moneyCost+" coins</button>"
        }
        html += "</div>"
